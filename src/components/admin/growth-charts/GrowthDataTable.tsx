...
export const GrowthDataTable = ({
  ...
  const renderEditableCell = (
    ...
    const isEditing =
      editingCell?.controlId === control.id && editingCell?.field === field;
  ...
  // Sort controls by date descending for display
  ...
  return (
    <div className="overflow-x-auto">
        ...
        <TableBody>
          {sortedControls.map((control) => (
            <TableRow key={control.id}>
              ...
              <TableCell>
                {control.ageInMonths !== undefined ? (
                  formatAgeDisplay(control.ageInMonths)
                ) : (
                  "-"
                )}
              </TableCell>
              <TableCell>
                {renderEditableCell(control, "weight", control.weight)}
              </TableCell>
              <TableCell>
                {renderEditableCell(control, "height", control.height)}
              </TableCell>
              <TableCell>
                {renderEditableCell(control, "head_circumference", control.head_circumference)}
              </TableCell>
              <TableCell>
                <Button
                  size="icon"
      ...
      <p className="text-xs text-muted-foreground mt-2">
        * Haga clic en cualquier valor para editarlo. Presione Enter para guardar o Escape para cancelar.
      </p>
    </div>
  );
};
